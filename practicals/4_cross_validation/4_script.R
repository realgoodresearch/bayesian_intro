...
# directories
basedir <- file.path(getwd(), "practicals", "4_cross_validation")
...
mod <- cmdstan_model(file.path(basedir, "4_model.stan"))
...
yhat <- yhat[,grepl("^y_hat", names(yhat))]

# residuals
resid <- matrix(NA, nrow = nrow(yhat), ncol = md$n_test)
for (i in 1:n_test) {
  resid[, i] <- (yhat[, paste0("y_hat[", i, "]")] - y_test[i]) / y_test[i]
}
...
inaccuracy <- apply(resid, 1, function(x) {
  mean(abs(x))
})
summary(inaccuracy)
hist(inaccuracy)
...
dat <- data.frame(
  obs = y_test,
  pred_mean = apply(yhat, 2, mean),
  pred_median = apply(yhat, 2, quantile, probs=c(0.5)),
  pred_lower = apply(yhat, 2, quantile, probs=c(0.025)),
  pred_upper = apply(yhat, 2, quantile, probs=c(0.975))
)
...
# add observed-predicted points with credible intervals
for (i in 1:n_test){
  points(
    x = dat$obs[i], 
    y = dat$pred_mean[i]
...
# coverage of 95% credible intervals of predicted values (yhat)
mean(dat$obs >= dat$pred_lower & dat$obs <= dat$pred_upper)
