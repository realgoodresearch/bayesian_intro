...
# probability density of marginal posterior for mu
#** Note:  Update "from" and "to" define the range of values you want to plot (below) *
density_mu <- density(fit$draws('mu'),
                      from = 4,
                      to = 6)
...
# extract the density values (i.e. the values plotted on the y-axis)
posterior_mu <- density_mu$y


# probability density of the prior given in the model 
#** Note: This should be updated to match the prior for mu in the model being assessed *
prior_mu <- dnorm(x = density_mu$x, 
                  mean = 0,
                  sd = 100)


# likelihood of the data given mu, i.e. L(x|mu) 
log_likelihood_mu <- sapply(density_mu$x, 
                        function(m) {
                          Lfun(mu = m, 
                               sigma = md$sigma_true)
                          }
...
likelihood_mu <- rescale_log_likelihoods(log_likelihood_mu, 
...
# posterior
plot(x=density_mu$x, y=posterior_mu, type='l', lwd=0, col='grey',
     xlab = 'Parameter value',
     ylab = 'Probability density or scaled Likelihood')

polygon(x = c(density_mu$x, rev(density_mu$x)),
