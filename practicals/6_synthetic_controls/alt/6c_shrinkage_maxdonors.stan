...
// horseshoe shrinkage
// fixed slab constraint on maximum donor weight
data {
...
transformed data {
  real c2 = 0.1; // slab cap on n_donors (try values between 0.1-few donors and 2.0-more donors)
}
parameters {
  real alpha;
  real phi;
...
transformed parameters {
  ...
  simplex[K] weights;
  vector[K] local_scales_reg;
  
  for (k in 1 : K) {
    local_scales_reg[k] = sqrt(
                               (square(c2) * square(local_scales[k]))
                               / (square(c2)
                                  + square(global_scale)
                                    * square(local_scales[k])));
  }
  
  weights_shrunk = weights_raw .* local_scales_reg * global_scale;
